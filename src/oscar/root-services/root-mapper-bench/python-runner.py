...
# Get a list of headers and shared libraries from MINIO.

headers = mc.list_objects(bucket_name, 'functions/includes/', recursive=True)

# Download headers and libraries
for header in headers:
    print(header)
    header = header.object_name.split('/')[-1]
    print(header)
    mc.fget_object(bucket_name, f'functions/headers/{header}', f'/opt/includes/{header}')

# All headers/libs are downloaded to the same paths,
# inform ROOT about these locations.
ROOT.gInterpreter.AddIncludePath("/opt/includes")
