...
class OSCARBackend(Base.BaseBackend):
    ...
    def __init__(self, oscarclient=None):
        ...
        try:
            ...
            print(self.client['mc'].bucket_exists(self.client['bucket_name']))
        except:
            ...
            self.client['mc'] = Minio(
                ...
                secure = False,
                http_client=urllib3.ProxyManager( # Despite insecure request force https
                    f"http://{self.client['minio_endpoint']}",
                    cert_reqs='CERT_NONE'
                )
            )
    ...
    def ProcessAndMerge(self, ranges, mapper, reducer):
        ...
        self.client['mapper_count'] = len(ranges)
    ...
    def _report_to_csv(self, data_dict):
        ...
        folder = f"{self.client['folder']}/" if 'folder' in self.client else ''
        # Generate CSV
        delimiter= '|'
        headings_process = [
